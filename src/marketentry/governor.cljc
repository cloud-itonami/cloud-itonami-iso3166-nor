(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Norway-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Norway-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-no-entity?
                                       true` (near-universal for Norway
                                       ...
                                       Grounded in Norwegian legal entity
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(ORGNR/統一資格/Doffin登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- no-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-no-entity? true`, INDEPENDENTLY verify
  `:has-no-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-no-entity?` ground truth (most Norway public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-no-entity? e))
                 (not (true? (:has-no-entity? e))))
        [{:rule :no-entity-missing
          :detail (str subject " はノルウェー居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (no-entity-missing-violations request st)

...
2. Register the client's intake: business type, target public function,
   prior filing history in Norway if any.
3. Run the advisor in read-only mode against Doffin.
4. Compare the checklist against the client's current documentation
   (Brønnøysund Register Centre status).
...
- named referral relationship with Norwegian-licensed counsel or a
...
a working referral relationship with Norwegian-licensed counsel or a
registered agent for whatever licensed representation the law of
Norway requires for actual public-procurement filings.

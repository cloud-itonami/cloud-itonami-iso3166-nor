# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Norway

## Classification

- Repository: `cloud-itonami-iso3166-nor`
- ISO 3166: `NOR` (Norway)
...
- Social impact: [:eea-single-market-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Norwegian
  public contract
...
- registration walkthrough for Doffin, Norway's national notification
  database for public procurement operated by DFØ (the Norwegian Agency
  for Public and Financial Management), the single entry point for
  all public-sector tender opportunities above national thresholds
- business/tax registration checklist: an entry in the Register of
  Business Enterprises maintained by the Brønnøysund Register Centre
  (a government body under the Ministry of Trade, Industry and
  Fisheries)
- local-content / preferential-procurement navigation: Norway is a
  European Economic Area (EEA) member (not an EU member state) — public
  procurement follows EU-directive-aligned open tendering under the EEA
  Agreement, no national-content quota, but Norwegian-language
  submission and a Nordic/EEA address for service are typically required
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Norwegian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Norway).
